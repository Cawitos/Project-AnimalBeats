...
def MascotasC():
    ...
    return render_template('Cliente/GestionMascotas.html', mascotas=mascotas)
...
def create_mascotV():
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            # Mensaje de éxito antes de redirigir
            flash("Mascota creada con éxito.")
            return redirect(url_for('mascotas_bp.MascotasV'))
...
def historialC(id_mascota):
    ...
    try:
        with connection.cursor() as cursor:
            ...
            if mascota_info is None:
                flash('No se encontró información de la mascota.', 'danger')
                return redirect(url_for('mascotas_bp.MascotasC'))
